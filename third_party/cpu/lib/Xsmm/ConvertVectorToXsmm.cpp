...
static std::optional<vector::ContractionOp>
rewriteGemmToBrgemm(PatternRewriter &rewriter,
                    vector::ContractionOp contractOp) {
  Location loc = contractOp.getLoc();
  MLIRContext *ctx = contractOp.getContext();

  // Check that it is a simple GEMM.
  // TODO: Extend rewrite to transposed GEMM variants.
  AffineExpr i, j, k;
  bindDims(ctx, i, j, k);
  ...
  // Assume that the loop's range and all pointer advances are known statically.
  // Thus, the induction variable should be unused.
  ...
  // Helper to match expected subgraph for input operand and get corresponding
  // loop argument.
  auto validateInputOperandAndGetBlockArg =
      [&](Value operand) -> std::optional<BlockArgument> {
    auto readOp = operand.getDefiningOp<vector::TransferReadOp>();
    if (!readOp)
      return std::nullopt;
    auto extractMemref = readOp.getDefiningOp<triton::cpu::ExtractMemRefOp>();
    if (!extractMemref || llvm::any_of(readOp.getIndices(), [](Value index) {
          return !index.getDefiningOp<triton::cpu::ExtractIndicesOp>();
        }))
      return std::nullopt;
    BlockArgument iterArg = dyn_cast<BlockArgument>(extractMemref.getSrc());
    return iterArg ? iterArg : std::nullopt;
  };

  BlockArgument lhsArg =
      validateInputOperandAndGetBlockArg(contractOp.getLhs());
  BlockArgument rhsArg =
      validateInputOperandAndGetBlockArg(contractOp.getRhs());
}
